...
public class DepartmentMenu extends AbstractMenuState {
  ...
  public boolean execute(Action a) throws IOException {
    ...
    if (a instanceof SelectMenuItem) {
      ...
      for (int i = 0; i < XMIT_BLOCKS_MAX && (len = _is.read(b)) > 0; i++) {
        _session.send(
            new TransmitData(
                b, len, i == XMIT_BLOCKS_MAX - 1 ? TransmitData.SAVE : TransmitData.SEND));
      }
      if (len < 0) {
      ...
      if (id == _iCurrParentID) {
        ...
        if (id == _iCurrMessageID) {
          ...
          if (id != _iCurrMenuID) selectMenu(id);
          pid = 0;
        }
  ...
  private void openStream(int id) throws IOException {
    ...
    try {
      ...
      rs =
          stmt.executeQuery(
              "SELECT filetype, LENGTH(data) as length,data from files where reference_id=" + id);
      if (rs.next()) {
        // get our File length.
  ...
  private void displayFileInfo(int id) throws IOException {
    ...
    try {
      ...
      rs =
          stmt.executeQuery(
              "SELECT name, filetype, description from files where reference_id=" + id);
      if (rs.next()) {
        ...
        rs =
            stmt.executeQuery(
                "SELECT reference_id FROM messages WHERE parent_id=" + id + " LIMIT 1");
        if (rs.next()) {
